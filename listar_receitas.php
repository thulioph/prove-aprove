...
function listar_ingredientes($receitaId,$conexao, $json){

  $query =  "select i.titulo,
              ri.quantidade
              from ingrediente i
              inner join receita_ingrediente ri on ri.ingrediente_id = i.id
              inner join receita r on r.id = ri.receita_id
              where r.id = $receitaId";


  $resultado = mysqli_query($conexao, $query);
  if(!empty($resultado)){
    $i="nospace";
    $json['dados'][0]->ingredientes = "";
    while($ingrediente = mysqli_fetch_object($resultado)){
      if($i == "nospace"){
        $json['dados'][0]->ingredientes = $ingrediente->titulo;
      }else{
        $json['dados'][0]->ingredientes = $json['dados'][0]->ingredientes . "," . $ingrediente->titulo;
      }
      //$json['dados']['ingredientes'][$i] = $ingrediente;
      $i = "space";
...
if($json['status'] == TRUE) {
  ...
  if(!empty($resultado)){
    ...
    //chamada para listagem de ingredientes
    if($filtro == 'receita'){
     $json = listar_ingredientes($id,$conexao,$json);
    }
  }
